...
# Minimum number of activities needed before trusting incremental sync.
# Below this threshold the DB was likely seeded by a verification step
# (e.g. list_activities --sync --limit 3) and a full-history fetch is safer.
_MIN_ACTIVITIES_FOR_INCREMENTAL = 20


def _activity_count(db_path: str) -> int:
    """Return total number of activities in the DB."""
    import sqlite3
    conn = sqlite3.connect(db_path)
    try:
...
def sync_summary(client: StravaClient, db_path: str, days: int | None = None,
    ...
    if days:
        ...
        count = _activity_count(db_path)
        if latest and count >= _MIN_ACTIVITIES_FOR_INCREMENTAL:
            after = latest - 2 * 86400  # 2-day overlap buffer
        # else: DB empty or too few activities → after stays None → fetch everything
    total = 0
