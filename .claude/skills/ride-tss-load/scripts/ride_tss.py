...
def main() -> None:
    ...
    try:
        ...
        snap = ensure_snapshot(db, required_fields=["ftp_w"])
        ftp = args.ftp or snap.get("ftp_w")
        if not ftp:
            output_error("No FTP available. Run ride-ftp-estimate first or pass --ftp.")
        ...
        for r in rides:
            ...
            i_f = intensity_factor(np_val, ftp)
            tss_val = tss(duration_s, np_val, i_f, ftp)
