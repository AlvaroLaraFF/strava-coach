...
26 skills total. Each skill has its own folder under `.claude/skills/<name>/`
...
| "consolida memoria / refresh memory / what do you remember" | memory-consolidate |
...
All analytics outputs are **recomputable on demand** from the raw `activities`
table + formulas in `analytics.py`. There is no snapshot or audit log table.

When the user asks **"how has my X evolved"** or **"compare with last month"**,
re-run the relevant skill with a wider `--days` window. The progression is in
the data, not in a cache.

Qualitative observations about the user's progression ("you've been
overreaching since early April") belong in **memory files**, not in a database.
See the Memory protocol section below.
...
**The user can also trigger consolidation explicitly** by saying:
- "consolida memoria"
- "actualiza mi perfil"
- "review my profile"
...
Save to:
`/home/alfernandez/.claude/projects/-home-alfernandez-PycharmProjects-strava-coach/memory/<filename>.md`
...
├── strava/                 # Shared library (client, db, analytics, sync)
├── scripts/                # Legacy root scripts (still functional, used by strava-coach legacy skill)
