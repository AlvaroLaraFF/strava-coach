...
│   ├── snapshot.py             #   Full snapshot computation
│   └── sync.py                 #   Incremental sync orchestration
│
├── .claude/skills/             # 27 Claude Code skills
│   ├── strava-setup/           #   First-time wizard + OAuth + profile scripts
│   ├── strava-sync/            #   Three-level data sync (summary → details → streams)
│   ├── strava-coach/           #   Legacy queries: list activities, stats, propose sessions
│   ├── athlete-snapshot/       #   Physiological profile + shared read/update snapshot scripts
...
│   └── ... (16 more)           #   One skill per question type
│
